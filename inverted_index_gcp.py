...
class MultiFileWriter:
    ...
    def upload_to_gcp(self,folder_name):
        ...
        blob = self.bucket.blob(f"postings_gcp/{folder_name}/{file_name}")
...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes,folder_name):
        b = []
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = open(f'postings_gcp/{folder_name}/{f_name}', 'rb')
...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        # store document length for each document
    ...
    def posting_lists_iter(self):
        ...
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.read(locs, self.df[w] * TUPLE_SIZE, self.folder_name)
                posting_list = []
    ...
    @staticmethod
    def read_index(base_dir, name, folder_name):
